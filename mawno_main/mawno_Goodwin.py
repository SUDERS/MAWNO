...
from config_Goodwin import Config as config
...
class train:
    ...
    def ode_gradient(self, x, y):
        ...
        X1_t = torch.gradient(X1, spacing=(self.config.t_torch,))[0]
        X2_t = torch.gradient(X2, spacing=(self.config.t_torch,))[0]
        Y1_t = torch.gradient(Y1, spacing=(self.config.t_torch,))[0]
        Y2_t = torch.gradient(Y2, spacing=(self.config.t_torch,))[0]

        dy0 = k.a1 / (
                k.A1 + k.k11 * Y1 + k.k12 * Y2) - k.b1
        dy1 = k.a2 / (
                k.A2 + k.k21 * Y1 + k.k22 * Y2) - k.b2
        dy2 = k.alp1 * X1 - k.bet1
        dy3 = k.alp2 * X2 - k.bet2
    ...
    def loss(self, y):
        ...
        ode_1, ode_2, ode_3, ode_4 = self.ode_gradient(self.config.x, y)
        # ode_1, ode_2 = ode_1*100, ode_2*100
        zeros_1D = torch.tensor([0.0] * self.config.T_N).to(self.config.device)

        loss1 = self.criterion(y0_pred, y0_true)
        loss2 = self.config.lambda2 * (self.criterion(ode_1, zeros_1D) + self.criterion(ode_2, zeros_1D)
                                       + self.criterion(ode_3, zeros_1D) + self.criterion(ode_4, zeros_1D))
    ...
    def real_loss(self, y):
        ...
        real_loss_nmse = torch.mean(self.criterion_non_reduce(y[0, :, :]+15, truth+15) / ((truth+15) ** 2))
        return real_loss_mse, real_loss_nmse

    def test_loss(self,y2):
        truth2 = torch.tensor(self.config.truth2[:, :]).to(self.config.device)
        test_loss_nmse = torch.mean(self.criterion_non_reduce(y2[0, :, :]+15, truth2+15) / ((truth2+15) ** 2))
